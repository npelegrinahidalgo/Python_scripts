...
######## Change the label of the headers file (filename_conatains) and the DBSCAN folder created in the for loop###########
...
for path in pathlist:
    ...
    plt.savefig(save_path +str(eps_threshold) + "_" + str(minimum_locs_threshold)+"Precision.pdf")
    plt.show()
    ...
    if to_cluster==1:
        ...
        if len(cluster_contents)>0:
            ...
            plt.savefig(save_path +str(eps_threshold) + "_" + str(minimum_locs_threshold)+ 'Localisations.pdf')
            plt.show()
            ...
            imsr.save(save_path +str(eps_threshold) + "_" + str(minimum_locs_threshold)+'Test_prec.tif')
            ...
            imsr.save(save_path +str(eps_threshold) + "_" + str(minimum_locs_threshold)+'SR_width_python_clustered.tif')
            
            imsr = Image.fromarray(labelled)
            imsr.save(save_path +str(eps_threshold) + "_" + str(minimum_locs_threshold)+'SR_fwhm_python_clustered.tif')
            ...
            plt.savefig(save_path +str(eps_threshold) + "_" + str(minimum_locs_threshold)+"Area.pdf")
            plt.show()
            ...
            plt.savefig(save_path +str(eps_threshold) + "_" + str(minimum_locs_threshold)+"Lengths.pdf")
            plt.show()
            ...
            plt.savefig(save_path +str(eps_threshold) + "_" + str(minimum_locs_threshold)+"Ecc.pdf")
            plt.show()
            ...
            if len(measurements)==len(cluster_contents):
                ...
                measurements.to_csv(save_path + 'Metrics.csv', sep = '\t')
            ...
            Output_overall.to_csv(save_path + str(eps_threshold) + "_" + str(minimum_locs_threshold)+'all.csv', sep = '\t')    
        ...
        if to_cluster==1:
            Output_all_cases.to_csv(save_path + str(eps_threshold) + '_' + str(minimum_locs_threshold) + 'clusters_data_GDSC_all_metrics.csv', sep = '\t')
