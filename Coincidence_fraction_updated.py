...
for path in pathlist:
    ...
    for row in rows:
        ...
        for well in wells:
            ...
            DBSCAN_folder = FOV + str("1_5_515_DBSCAN_eccentricity_test/")
            ...
            # Coincidence analysis (without function)
            
            features,labelled_image1= label_image(NR_binary)
            ...
            fraction_coinc = coinc_total/features
            
            
            
        
            fraction_rounded = round(fraction_coinc, 10)
            ...
            EV_table = EV_table.append({"File":path, "Number of EVs": int(features),"Coincidence": int(coinc_total), "Fraction coincidence": fraction_rounded, "ID": str(path[65] + path[66])},ignore_index=True)
...
EV_table.to_csv(root_path + "Coincidence_per_EV_ecc_filtered.csv", sep = "\t")
